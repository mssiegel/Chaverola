...
| 23 · Hebrew            | **Complete** 2026-07-27 — `/he` is a Hebrew product, not an English one translated: every string, right-to-left layout, a Hebrew transcript email, an activity that remembers its language and hands it to joining students, and a demo cast an Israeli teacher recognizes.            | [hebrew](docs/plans/hebrew/README.md)                                |
...
- Files: [client/src/i18n/](client/src/i18n/) (`locales/{en,he}/<ns>.ts` catalogs, `ns/<ns>.ts` registrars, `i18next.d.ts`), `lib/locale.ts` (URL helpers; re-exports the locale list from `shared/`), `lib/localePreference.ts` + `lib/localeBoot.ts` (detection), `components/layout/LocaleEffects.tsx`, `pages/student/JoinActivityPage.tsx` (the activity rung: a resolved join code swaps the URL's locale, before a seat exists and never after).
