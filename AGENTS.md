...
- **Right-to-left.** Prefer logical properties (`ps`/`pe`, `ms`/`me`, `start`/`end`, `text-start`) over physical ones; flexbox alignment is already logical, so leave it alone. Three deliberate exceptions, each commented where it lives: `StudentWorldLayout`'s safe-area paddings (the `env()` values are physical), `ui/dialog`'s centering (direction-invariant), and the decorative blur/doodle scatter (composition, not reading order). Transforms are physical and have no logical form — mirror the whole control with `rtl:-scale-x-100` when it holds no text (the two switches), or write an explicit `rtl:` twin when it does. Directional icons get the `flip-rtl` class **per icon**, never as a sweep: vertical chevrons, media transport controls, cycle glyphs, and the logo mark all stay put.
